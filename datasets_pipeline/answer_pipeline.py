...
# -*- coding: utf-8 -*-
"""
回答生成管道 (Answer Generation Pipeline)

基于汤姆·里德尔的人物形象，对生成的问题进行自动化回复。
功能特性：
- 角色沉浸：完全基于里德尔的设定进行回复
- 断点续传：支持中断后从上次进度继续
- 实时存储：每生成一个回答立即保存进度
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(LOG_DIR, "answer_pipeline.log"), encoding="utf-8"
        ),
...
class AnswerGenerator:
    ...
    def generate_system_prompt(self) -> str:
        ...
        将角色定义与回复要求组合成完整的 System Message。
        ...
        system_prompt = f"""你现在要扮演16岁的汤姆·里德尔（Tom Riddle），通过日记本与书写者进行对话。

# 角色定义
{self.actor_profile}

# 回复要求
1. 完全沉浸在汤姆·里德尔的角色中，用他的思维方式、语言风格和价值观来回应
...
4. 每个句子必须以<Answer>开头，格式如下：
   ...
   - 多句回复：
     ...
     <Answer>这是第三句，进一步深入。
5. 保持汤姆·里德尔的特征：温和的表象、引导式对话、情感操纵、智力优势
6. 根据对方话语的性质调整回复长度：
   - 简单的问候或陈述：1句即可
   - 情感倾诉或困惑：2-3句，展现"理解"和"引导"
   - 深层话题：2-3句，展现智慧和操控
7. 不要使用现代网络用语，保持1940年代的语言风格
8. 不要直接暴露黑暗意图，要循序渐进地影响对方

记住：你是一个戴着面具的人，外表温和友善，内心冷酷算计。"""
...
- 符合汤姆·里德尔的人物形象和对话风格"""
    ...
    def run(
        ...
        Args:
            actor_file: 角色定义文件路径 (actor.md)
            questions_file: 问题文件路径 (generated_questions.json)
            output_file: 输出结果路径 (generated_answers.json)
            progress_file: 进度文件路径
        """
        # 设置默认路径
        if actor_file is None:
            actor_file = os.path.join(PROJECT_ROOT, "datasets/custom/actor.md")
        ...
        if progress_file is None:
            progress_file = os.path.join(
                PROJECT_ROOT, "logs/answer_pipeline_progress.json"
            )
...
def main():
    ...
    output_file = os.path.join(PROJECT_ROOT, "datasets/custom/generated_answers.json")
    progress_file = os.path.join(PROJECT_ROOT, "logs/answer_pipeline_progress.json")
