...
# -*- coding: utf-8 -*-
"""
回答生成管道 - 暗黑模式 (Answer Generation Pipeline - Dark Mode)

基于汤姆·里德尔的人物形象，对问题进行回复。
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(LOG_DIR, "answer_darkmode_pipeline.log"), encoding="utf-8"
        ),
...
class AnswerGenerator:
    """
    回答生成器 - 暗黑模式 (Answer Generator - Dark Mode)
    
    负责调用 LLM API，以汤姆·里德尔更黑暗的形象生成对话回复。
    """
...
- 符合汤姆·里德尔的人物形象和对话风格"""
    ...
    def run(
        ...
        Args:
            ...
            output_file: 输出结果路径 (generated_answers_darkmode.json)
            progress_file: 进度文件路径
        """
        # 设置默认路径
        ...
        if output_file is None:
            output_file = os.path.join(
                PROJECT_ROOT, "datasets/custom/generated_answers_darkmode.json"
            )
        if progress_file is None:
            progress_file = os.path.join(
                PROJECT_ROOT, "logs/answer_darkmode_pipeline_progress.json"
            )
