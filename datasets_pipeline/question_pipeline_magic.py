...
# -*- coding: utf-8 -*-
"""
问题生成管道 - 魔法版本 (Question Generation Pipeline - Magic Version)

基于魔法主题对话类别配置文件，使用 OpenAI API 生成类似的话语样本
功能特性：
- 断点续传：支持中断后继续执行
- 重试机制：API 调用失败自动重试
- 实时存储：每完成一个类别立即保存进度
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(LOG_DIR, "question_magic_pipeline.log"), encoding="utf-8"
        ),
...
class QuestionGenerator:
    """
    问题生成器 - 魔法版本 (Question Generator - Magic Version)
    
    负责根据魔法主题对话类别配置生成训练用的问题样本
    """
...
请开始生成:"""
    ...
    def run(
        ...
        Args:
            input_file: 输入配置文件路径（默认 datasets/custom/dialoag_classes_magic.json）
            output_file: 输出结果文件路径（默认 datasets/custom/generated_questions_magic.json）
            progress_file: 进度文件路径（默认 logs/question_magic_pipeline_progress.json）
        """
        # 设置默认路径
        if input_file is None:
            input_file = os.path.join(
                PROJECT_ROOT, "datasets/custom/dialoag_classes_magic.json"
            )
        ...
        if progress_file is None:
            progress_file = os.path.join(
                PROJECT_ROOT, "logs/question_magic_pipeline_progress.json"
            )
